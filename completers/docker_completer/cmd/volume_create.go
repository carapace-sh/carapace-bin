...
func init() {
	...
	volume_createCmd.Flags().Int("limit-bytes", 0, "Minimum size of the Cluster Volume in bytes")
	volume_createCmd.Flags().String("name", "", "Specify volume name")
	volume_createCmd.Flags().StringP("opt", "o", "", "Set driver specific options")
	volume_createCmd.Flags().Int("required-bytes", 0, "Maximum size of the Cluster Volume in bytes")
