...
func init() {
	...
	service_createCmd.Flags().Int("limit-cpu", 0, "Limit CPUs")
	service_createCmd.Flags().Int("limit-memory", 0, "Limit Memory")
	...
	service_createCmd.Flags().String("mode", "replicated", "Service mode (replicated, global, replicated-job, or global-job)")
	...
	service_createCmd.Flags().Int("reserve-cpu", 0, "Reserve CPUs")
	service_createCmd.Flags().Int("reserve-memory", 0, "Reserve Memory")
	...
	service_createCmd.Flags().Uint64("rollback-parallelism", 1, "Maximum number of tasks rolled back simultaneously (0 to roll back all at once)")
	...
	service_createCmd.Flags().Uint64("update-parallelism", 1, "Maximum number of tasks updated simultaneously (0 to update all at once)")
