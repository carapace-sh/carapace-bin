...
import (
	...
	"github.com/carapace-sh/carapace-bin/completers/rg_completer/cmd/action"
...
func init() {
	...
	causes . to match line terminators when multiline searching is enabled
	...
	rootCmd.Flags().String("pre-glob", "", "Works in conjunction with the --pre flag.")
	...
	rootCmd.Flags().String("pre", "", "For each input FILE, search the standard output of COMMAND FILE instead of its contents.")
	rootCmd.Flags().String("pre-glob", "", "Works with --pre flag to limit which files are transformed.")
	rootCmd.Flags().Bool("no-require-git", false, "Respect source control ignore files without requiring a git repository.")
	rootCmd.Flags().Bool("one-file-system", false, "Do not cross file system boundaries relative to the search starting point.")
	rootCmd.Flags().Bool("no-config", false, "Never read configuration files.")
	rootCmd.Flags().Bool("no-ignore-messages", false, "Suppress error messages related to parsing ignore files.")
