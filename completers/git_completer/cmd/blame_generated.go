...
func init() {
	...
	blameCmd.Flags().BoolS("b", "b", false, "Show blank SHA-1 for boundary commits (Default: off)")
	...
	blameCmd.Flags().BoolS("c", "c", false, "Use the same output mode as git-annotate (Default: off)")
	...
	blameCmd.Flags().StringS("L", "L", "", "Process only line range n,m, counting from 1")
	blameCmd.Flags().BoolS("l", "l", false, "Show long commit SHA1 (Default: off)")
	...
	blameCmd.Flags().BoolS("s", "s", false, "Suppress author name and timestamp (Default: off)")
	blameCmd.Flags().BoolS("t", "t", false, "Show raw timestamp (Default: off)")
