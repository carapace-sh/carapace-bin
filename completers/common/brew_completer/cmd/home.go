...
func init() {
	...
	carapace.Gen(homeCmd).PositionalAnyCompletion(
		carapace.Batch(
			brew.ActionAllCasks().Unless(homeCmd.Flag("formula").Changed),
			brew.ActionAllFormulae().Unless(homeCmd.Flag("cask").Changed),
		).ToA().FilterArgs(),
	)
}
