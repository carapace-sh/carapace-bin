...
func init() {
	...
	installCmd.Flags().Bool("as-dependency", false, "Install but mark as installed as a dependency.")
	...
	installCmd.Flags().String("bottle-arch", "", "Optimise bottles for the specified architecture rather than the oldest architecture supported by the version of macOS the bottles are built on.")
	...
	installCmd.Flags().String("cc", "", "Attempt to compile using the specified <compiler>, which should be the name of the compiler's executable, e.g. `gcc-7` for GCC 7. In order to use LLVM's clang, specify `llvm_clang`. To use the Apple-provided clang, specify `clang`. This option will only accept compilers that are provided by Homebrew or bundled with macOS. Please do not file issues if you encounter errors while using this option.")
	...
	installCmd.Flags().BoolS("dry-run", "n", false, "Show what would be installed, but do not actually install anything.")
	...
	installCmd.Flags().String("language", "", "Comma-separated list of language codes to prefer for cask installation. The first matching language is used, otherwise it reverts to the cask's default language. The default value is the language of your system.")
	installCmd.Flags().String("mdimporterdir", "", "Target location for Spotlight Plugins (default: `~/Library/Spotlight`).")
	installCmd.Flags().BoolS("no-ask", "y", false, "Do not ask for confirmation before downloading and installing.")
	...
	installCmd.Flags().Bool("skip-link", false, "Skip linking into prefix.")
