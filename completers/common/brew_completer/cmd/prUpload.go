...
func init() {
	...
	prUploadCmd.Flags().String("committer", "", "Specify a committer name and email in `git`'s standard author format.")
	...
	prUploadCmd.Flags().String("root-url", "", "Use the specified <URL> as the root of the bottle's URL instead of Homebrew's default.")
	prUploadCmd.Flags().String("root-url-using", "", "Use the specified download strategy class for downloading the bottle's URL instead of Homebrew's default.")
