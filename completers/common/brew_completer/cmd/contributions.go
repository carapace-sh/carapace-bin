...
func init() {
	...
	contributionsCmd.Flags().String("from", "", "Date (ISO-8601 format) to start searching contributions. Omitting this flag searches the last year.")
	...
	contributionsCmd.Flags().String("repositories", "", "Specify a comma-separated list of repositories to search. Supported repositories: `brew`, `core`, `cask`, `aliases`, `autoupdate`, `bundle`, `command-not-found`, `test-bot`, `services`, `cask-fonts` and `cask-versions`. Omitting this flag, or specifying `--repositories=primary`, searches only the main repositories: brew,core,cask. Specifying `--repositories=all`, searches all repositories. ")
	contributionsCmd.Flags().String("to", "", "Date (ISO-8601 format) to stop searching contributions.")
	contributionsCmd.Flags().String("user", "", "Specify a comma-separated list of GitHub usernames or email addresses to find contributions from. Omitting this flag searches maintainers.")
