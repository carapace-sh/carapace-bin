...
func init() {
	...
	history_listCmd.Flags().StringP("format", "f", "", "Available variables: {command}, {directory}, {duration}, {user}, {host}, {author}, {intent}, {exit}, {time}, {session}, and {uuid} Example: --format \"{time} - [{duration}] - {directory}$\\t{command}\"")
