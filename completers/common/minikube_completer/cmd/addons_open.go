...
func init() {
	...
	addons_openCmd.PersistentFlags().String("format", "", "Format to output addons URL in.  This format will be applied to each url individually and they will be printed one at a time.")
	addons_openCmd.Flags().Bool("https", false, "Open the addons URL with https instead of http")
	addons_openCmd.Flags().String("interval", "", "The time interval for each check that wait performs in seconds")
	...
	addons_openCmd.Flags().String("wait", "", "Amount of time to wait for service in seconds")
