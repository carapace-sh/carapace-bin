...
func init() {
	...
	logsCmd.Flags().Bool("audit", false, "Show only the audit logs")
	...
	logsCmd.Flags().Bool("last-start-only", false, "Show only the last start logs.")
	logsCmd.Flags().StringP("length", "n", "", "Number of lines back to go within the log")
