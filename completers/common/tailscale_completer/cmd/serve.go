...
func init() {
	...
	serveCmd.Flags().String("http", "", "expose an HTTP server at the specified port")
	serveCmd.Flags().String("https", "", "expose an HTTPS server at the specified port")
	serveCmd.Flags().String("proxy-protocol", "", "PROXY protocol version (1 or 2) for TCP forwarding")
	...
	serveCmd.Flags().String("tcp", "", "expose a TCP forwarder to forward raw TCP packets at the specified port")
	serveCmd.Flags().String("tls-terminated-tcp", "", "expose a TCP forwarder to forward TLS-terminated TCP packets at the specified port")
