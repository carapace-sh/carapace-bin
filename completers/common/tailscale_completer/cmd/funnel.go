...
func init() {
	...
	funnelCmd.Flags().String("https", "", "expose an HTTPS server at the specified port")
	funnelCmd.Flags().String("proxy-protocol", "", "PROXY protocol version (1 or 2) for TCP forwarding")
	funnelCmd.Flags().String("set-path", "", "append the specified path to the base URL for accessing the underlying service")
	funnelCmd.Flags().String("tcp", "", "expose a TCP forwarder to forward raw TCP packets at the specified port")
	funnelCmd.Flags().String("tls-terminated-tcp", "", "expose a TCP forwarder to forward TLS-terminated TCP packets at the specified port")
