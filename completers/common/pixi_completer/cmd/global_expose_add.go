...
func init() {
	...
	global_expose_addCmd.Flags().Bool("no-hard-links", false, "Disallow hard links during package installation")
	global_expose_addCmd.Flags().Bool("no-ref-links", false, "Disallow ref links (copy-on-write) during package installation")
	global_expose_addCmd.Flags().Bool("no-symbolic-links", false, "Disallow symbolic links during package installation")
	global_expose_addCmd.Flags().Bool("offline", false, "Run without network access, using only cached data. Commands fail if data is missing from the cache. Pass `--offline=false` to override an `offline` setting from the configuration")
	...
	global_expose_addCmd.Flags().String("tls-root-certs", "", "Which TLS root certificates to use: 'webpki' (bundled Mozilla roots) or 'system' (system store)")
