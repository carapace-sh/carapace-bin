...
func init() {
	...
	runCmd.Flags().String("config-file", "", "Load configuration from this file instead of searching system and user-level paths. Project-local `<project>/.pixi/config.toml` is still merged on top")
	...
	runCmd.Flags().BoolP("executable", "x", false, "Execute the command as an executable without resolving Pixi tasks")
	...
	runCmd.Flags().Bool("frozen", false, "Install the environment as defined in the lock file, doesn't update lock file if it isn't up-to-date with the manifest file")
	...
	runCmd.Flags().Bool("no-hard-links", false, "Disallow hard links during package installation")
	runCmd.Flags().Bool("no-install", false, "Don't modify the environment, only modify the lock file")
	runCmd.Flags().Bool("no-ref-links", false, "Disallow ref links (copy-on-write) during package installation")
	runCmd.Flags().Bool("no-symbolic-links", false, "Disallow symbolic links during package installation")
	...
	runCmd.Flags().String("tls-root-certs", "", "Which TLS root certificates to use: 'webpki' (bundled Mozilla roots) or 'system' (system store)")
	runCmd.Flags().Bool("use-environment-activation-cache", false, "Use environment activation cache (experimental)")
	runCmd.PersistentFlags().StringP("workspace", "w", "", "Name of the workspace")
