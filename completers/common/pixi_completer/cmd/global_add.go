...
func init() {
	...
	global_addCmd.Flags().String("build-backend", "", "The build backend to build the source with, when the source does not provide its own package manifest (or to override the one it has)")
	...
	global_addCmd.Flags().Bool("no-hard-links", false, "Disallow hard links during package installation")
	global_addCmd.Flags().Bool("no-ref-links", false, "Disallow ref links (copy-on-write) during package installation")
	global_addCmd.Flags().Bool("no-symbolic-links", false, "Disallow symbolic links during package installation")
	global_addCmd.Flags().Bool("offline", false, "Run without network access, using only cached data. Commands fail if data is missing from the cache. Pass `--offline=false` to override an `offline` setting from the configuration")
	global_addCmd.Flags().String("package", "", "Additional fields of the inline package definition, as `DOTTED_KEY=TOML_VALUE` pairs that are recorded under the `package` key of the dependency")
	...
	global_addCmd.Flags().String("subdirectory", "", "The subdirectory within the git repository")
	...
	global_addCmd.Flags().String("tls-root-certs", "", "Which TLS root certificates to use: 'webpki' (bundled Mozilla roots) or 'system' (system store)")
