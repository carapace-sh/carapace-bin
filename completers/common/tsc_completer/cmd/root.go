...
func init() {
	...
	rootCmd.Flags().Bool("allowArbitraryExtensions", false, "Enable importing files with any extension, provided a declaration file is present.")
	rootCmd.Flags().Bool("allowImportingTsExtensions", false, "Allow imports to include TypeScript file extensions. Requires '--moduleResolution bundler' and either '--noEmit' or '--emitDeclarationOnly' to be set.")
	...
	rootCmd.Flags().String("charset", "", "No longer supported. In early versions, manually set the text encoding for reading files.")
	...
	rootCmd.Flags().String("customConditions", "", "Conditions to set in addition to the resolver-specific defaults when resolving imports.")
	...
	rootCmd.Flags().Bool("erasableSyntaxOnly", false, "Do not allow runtime constructs that are not part of ECMAScript.")
	...
	rootCmd.Flags().Bool("isolatedDeclarations", false, "Require sufficient annotation on exports so other tools can trivially generate declaration files.")
	...
	rootCmd.Flags().Bool("keyofStringsOnly", false, "Make keyof only return strings instead of string, numbers or symbols. Legacy option.")
	rootCmd.Flags().String("lib", "", "Specify a set of bundled library declaration files that describe the target runtime environment.")
	rootCmd.Flags().Bool("libReplacement", false, "Enable lib replacement.")
	...
	rootCmd.Flags().String("moduleDetection", "", "Control what method is used to detect module-format JS files.")
	rootCmd.Flags().String("moduleResolution", "", "Specify how TypeScript looks up a file from a given module specifier.")
	rootCmd.Flags().String("moduleSuffixes", "", "List of file name suffixes to search when resolving a module.")
	rootCmd.Flags().String("newLine", "", "Set the newline character for emitting files.")
	rootCmd.Flags().Bool("noCheck", false, "Disable full type checking (only critical parse and emit errors will be reported).")
	...
	rootCmd.Flags().Bool("noImplicitUseStrict", false, "Disable adding 'use strict' directives in emitted JavaScript files.")
	...
	rootCmd.Flags().Bool("noStrictGenericChecks", false, "Disable strict checking of generic signatures in function types.")
	rootCmd.Flags().Bool("noUncheckedIndexedAccess", false, "Include 'undefined' in index signature results")
	rootCmd.Flags().Bool("noUncheckedSideEffectImports", false, "Check side effect imports.")
	...
	rootCmd.Flags().String("out", "", "Deprecated setting. Use 'outFile' instead.")
	...
	rootCmd.Flags().Bool("preserveValueImports", false, "Preserve unused imported values in the JavaScript output that would otherwise be removed.")
	...
	rootCmd.Flags().Bool("resolvePackageJsonExports", false, "Use the package.json 'exports' field when resolving package imports.")
	rootCmd.Flags().Bool("resolvePackageJsonImports", false, "Use the package.json 'imports' field when resolving imports.")
	rootCmd.Flags().Bool("rewriteRelativeImportExtensions", false, "Rewrite '.ts', '.tsx', '.mts', and '.cts' file extensions in relative import paths to their JavaScript equivalent in output files.")
	...
	rootCmd.Flags().Bool("stopBuildOnErrors", false, "Skip building downstream projects on error in upstream project.")
	...
	rootCmd.Flags().Bool("strictBuiltinIteratorReturn", false, "Built-in iterators are instantiated with a 'TReturn' type of 'undefined' instead of 'any'.")
	...
	rootCmd.Flags().Bool("suppressExcessPropertyErrors", false, "Disable reporting of excess property errors during the creation of object literals.")
	rootCmd.Flags().Bool("suppressImplicitAnyIndexErrors", false, "Suppress 'noImplicitAny' errors when indexing objects that lack index signatures.")
	...
	rootCmd.Flags().Bool("verbatimModuleSyntax", false, "Do not transform or elide any imports or exports not marked as type-only, ensuring they are written in the output file's format based on the 'module' setting.")
	...
	carapace.Gen(rootCmd).FlagCompletion(carapace.ActionMap{
		...
		"moduleDetection":  carapace.ActionValues("legacy", "auto", "force"),
