...
func init() {
	...
	proxy_awsCmd.Flags().StringP("format", "f", "unix", "Optional format to print the commands for setting environment variables, one of: unix, command-prompt, powershell, text. Default is unix. Or specify a service format, one of: athena-odbc, athena-jdbc.")
	...
	proxy_awsCmd.Flag("endpoint-url").Hidden = true
