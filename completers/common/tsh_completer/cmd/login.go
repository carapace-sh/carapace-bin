...
func init() {
	...
	loginCmd.Flags().Bool("force", false, "Force re-authentication even if the current session is still valid.")
	loginCmd.Flags().StringP("format", "f", "file", "Identity format: file, openssh (for OpenSSH compatibility) or kubernetes (for kubeconfig).")
	loginCmd.Flags().String("kube-cluster", "", "Name of the Kubernetes cluster to login to.")
	loginCmd.Flags().Bool("no-force", false, "Force re-authentication even if the current session is still valid.")
	...
	loginCmd.Flags().String("scope", "", "Scope pins credentials to a given scope. Use \"\" to explicitly remove scoping.")
	loginCmd.Flags().BoolP("verbose", "v", false, "Show extra status information.")
	loginCmd.Flag("no-force").Hidden = true
