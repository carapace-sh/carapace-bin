...
func init() {
	...
	driveCmd.Flags().StringArrayP("android-project-arg", "P", nil, "Additional arguments specified as key=value that are passed directly to the gradle project via the -P flag.")
	...
	driveCmd.Flags().StringArrayP("dart-entrypoint-args", "a", nil, "Pass a list of arguments to the Dart entrypoint at application startup.")
	...
	driveCmd.Flags().String("flavor", "", "Build a custom app flavor as defined by platform-specific build setup.")
	...
	driveCmd.Flags().StringArray("test-arguments", nil, "Additional arguments to pass to the Dart VM running the test script.")
