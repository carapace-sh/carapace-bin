...
func init() {
	...
	assembleCmd.Flags().StringArrayP("dart-define", "D", nil, "Additional key-value pairs that will be available as constants.")
	assembleCmd.Flags().StringArrayP("define", "d", nil, "Allows passing configuration to a target, as in \"--define=target=key=value\".")
	...
	assembleCmd.Flags().StringArrayP("input", "i", nil, "Allows passing additional inputs with \"--input=key=value\".")
