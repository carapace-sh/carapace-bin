...
func init() {
	...
	build_iosCmd.Flags().String("flavor", "", "Build a custom app flavor as defined by platform-specific build setup.")
	...
	build_iosCmd.Flags().Bool("no-codesign", false, "Do not codesign the application bundle (only available on device builds).")
	build_iosCmd.Flags().Bool("no-config-only", false, "Do not update the project configuration without performing a build.")
	...
	build_iosCmd.Flags().Bool("no-obfuscate", false, "In a release build, this flag does not remove identifiers and replaces them with randomized values for the purposes of source code obfuscation.")
	build_iosCmd.Flags().Bool("no-pub", false, "Whether to run \"flutter pub get\" before executing this command.")
	build_iosCmd.Flags().Bool("no-simulator", false, "Do not build for the iOS simulator instead of the device.")
	build_iosCmd.Flags().Bool("no-tree-shake-icons", false, "Do not tree shake icon fonts so that only glyphs used by the application remain.")
