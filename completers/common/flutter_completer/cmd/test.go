...
func init() {
	...
	testCmd.Flags().StringArrayP("exclude-tags", "x", nil, "Run only tests that do not have the specified tags.")
	testCmd.Flags().Bool("fail-fast", false, "Stop tests after first failure.")
	...
	testCmd.Flags().StringArray("name", nil, "A regular expression matching substrings of the names of tests to run.")
	...
	testCmd.Flags().StringArray("plain-name", nil, "A plain-text substring of the names of tests to run.")
	...
	testCmd.Flags().StringArrayP("tags", "t", nil, "Run only tests associated with the specified tags.")
	testCmd.Flags().Bool("test-assets", false, "Build the assets bundle for testing.")
	testCmd.Flags().String("test-randomize-ordering-seed", "", "The seed to randomize the execution order of test cases within test files.")
	...
	testCmd.Flags().String("web-renderer", "", "The renderer implementation to use when building for the web.")
	...
	carapace.Gen(testCmd).FlagCompletion(carapace.ActionMap{
		...
		"reporter": carapace.ActionValuesDescribed(
			...
			"github", "A GitHub Actions-compatible format.",
