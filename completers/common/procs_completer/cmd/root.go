...
func init() {
	...
	rootCmd.Flags().Bool("debug", false, "Show debug message")
	...
	rootCmd.Flags().Bool("gen-man-page", false, "Generate man page and write to stdout")
	...
	rootCmd.Flags().Bool("regex", false, "Use regex search mode (single PATTERN)")
	rootCmd.Flags().Bool("smart", false, "Use smart mode (auto-detect regex syntax)")
	rootCmd.Flags().String("sorta", "", "Sort column by ascending")
	rootCmd.Flags().String("sortd", "", "Sort column by descending")
	rootCmd.Flags().Bool("text", false, "Use plain text search mode (default)")
