...
func init() {
	...
	rootCmd.Flags().Bool("best", false, "compress best (can be slow for big files)")
	...
	rootCmd.Flags().Bool("catch-sigsegv", false, "debug errors in hardware or de-compressor")
	...
	rootCmd.Flags().Bool("fileinfo", false, "show parameters of already-compressed file")
	rootCmd.Flags().Bool("force-overwrite", false, "force overwrite of output files")
	...
	rootCmd.Flags().Bool("link", false, "preserve hard links (Unix only) [USE WITH CARE]")
	rootCmd.Flags().Bool("lzma", false, "try LZMA [slower but tighter than NRV]")
	...
	rootCmd.Flags().Bool("no-link", false, "do not preserve hard links but rename files [default]")
	rootCmd.Flags().Bool("no-mode", false, "do not preserve file mode (aka permissions)")
	rootCmd.Flags().Bool("no-owner", false, "do not preserve file ownership")
	...
	rootCmd.Flags().Bool("no-time", false, "do not preserve file timestamp")
