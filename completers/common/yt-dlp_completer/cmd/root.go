...
func init() {
	...
	rootCmd.Flags().String("color", "", "Whether to emit color codes in output")
	...
	rootCmd.Flags().String("external-downloader-args", "", "Give these arguments to the external downloader")
	...
	rootCmd.Flags().Bool("force-download-archive", false, "Force the download archive to be written to the given path")
	...
	rootCmd.Flags().Bool("format-sort-reset", false, "Disregard previous user specified sort order and reset to the default")
	...
	rootCmd.Flags().String("impersonate", "", "Client to impersonate for requests")
	rootCmd.Flags().String("js-runtimes", "", "Additional JavaScript runtime to enable")
	...
	rootCmd.Flags().Bool("list-impersonate-targets", false, "List available clients to impersonate")
	...
	rootCmd.Flags().String("netrc-cmd", "", "Command to execute to get the credentials for an extractor")
	...
	rootCmd.Flags().Bool("no-add-chapters", false, "Do not add chapter markers to the video file")
	...
	rootCmd.Flags().Bool("no-break-on-existing", false, "Do not stop the download process when encountering a file that is in the archive")
	...
	rootCmd.Flags().Bool("no-match-filters", false, "Do not use any --match-filters (default)")
	...
	rootCmd.Flags().Bool("no-plugin-dirs", false, "Clear plugin directories to search")
	...
	rootCmd.Flags().Bool("no-quiet", false, "Deactivate quiet mode")
	rootCmd.Flags().Bool("no-remote-components", false, "Disallow fetching of all remote components")
	...
	rootCmd.Flags().String("plugin-dirs", "", "Path to an additional directory to search for plugins")
	...
	rootCmd.Flags().String("preset-alias", "", "Alias for a preset configuration")
	...
	rootCmd.Flags().String("progress-delta", "", "Time between progress output (default: 0)")
	...
	rootCmd.Flags().Bool("remote-components", false, "Remote components to allow yt-dlp to fetch when required")
	...
	rootCmd.Flags().String("xff", "", "How to fake X-Forwarded-For HTTP header to try bypassing geographic restriction")
