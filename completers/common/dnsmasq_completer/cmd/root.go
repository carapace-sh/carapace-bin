...
func init() {
	...
	rootCmd.Flags().String("add-subnet", "", "Add specified IP subnet to forwarded DNS queries.")
	...
	rootCmd.Flags().String("cache-rr", "", "Cache this DNS resource record type.")
	...
	rootCmd.Flags().String("conf-script", "", "Execute file and read configuration from stdin.")
	rootCmd.Flags().String("connmark-allowlist", "", "Set allowed DNS patterns for a connection-track mark.")
	rootCmd.Flags().Bool("connmark-allowlist-enable", false, "Enable filtering of DNS queries with connection-track marks.")
	...
	rootCmd.Flags().String("dhcp-option-pxe", "", "DHCP option sent only to PXE clients.")
	...
	rootCmd.Flags().String("dhcp-split-relay", "", "Relay DHCP requests to a remote server.")
	...
	rootCmd.Flags().String("dnssec-limits", "", "Set resource limits for DNSSEC validation.")
	...
	rootCmd.Flags().Bool("do-0x20-encode", false, "Enable DNS bit 0x20 encoding.")
	...
	rootCmd.Flags().String("dynamic-host", "", "Specify host record in interface subnet.")
	...
	rootCmd.Flags().String("fast-dns-retry", "", "Retry DNS queries after this many milliseconds.")
	rootCmd.Flags().Bool("filter-A", false, "Don't include IPv4 addresses in DNS answers.")
	rootCmd.Flags().Bool("filter-AAAA", false, "Don't include IPv6 addresses in DNS answers.")
	rootCmd.Flags().String("filter-rr", "", "Don't include resource records of the given type in DNS answers.")
	...
	rootCmd.Flags().Bool("leasequery", false, "Enable RFC 4388 leasequery functions for DHCPv4.")
	...
	rootCmd.Flags().Bool("log-malloc", false, "Log memory allocation for debugging.")
	...
	rootCmd.Flags().String("max-tcp-connections", "", "Maximum number of concurrent tcp connections.")
	...
	rootCmd.Flags().String("nftset", "", "Specify nftables sets to which matching domains should be added.")
	rootCmd.Flags().Bool("no-0x20-encode", false, "Suppress DNS bit 0x20 encoding.")
	...
	rootCmd.Flags().String("no-dhcpv4-interface", "", "Do not provide DHCPv4 on this interface.")
	rootCmd.Flags().String("no-dhcpv6-interface", "", "Do not provide DHCPv6 on this interface.")
	rootCmd.Flags().BoolP("no-hosts", "h", false, "Do NOT load /etc/hosts file.")
	rootCmd.Flags().Bool("no-ident", false, "Do not add CHAOS TXT records.")
	...
	rootCmd.Flags().Bool("no-round-robin", false, "Suppress round-robin ordering of DNS records.")
	...
	rootCmd.Flags().String("port-limit", "", "Set maximum number of random originating ports for a query.")
	...
	rootCmd.Flags().Bool("quiet-tftp", false, "Do not log routine TFTP.")
	rootCmd.Flags().String("ra-param", "", "Set MTU, priority, resend-interval and router-lifetime for RA.")
	...
	rootCmd.Flags().Bool("strip-mac", false, "Strip MAC information from queries.")
	rootCmd.Flags().Bool("strip-subnet", false, "Strip ECS information from queries.")
	...
	rootCmd.Flags().String("umbrella", "", "Send Cisco Umbrella identifiers including remote IP.")
	rootCmd.Flags().Bool("use-stale-cache", false, "Use expired cache data for faster reply.")
