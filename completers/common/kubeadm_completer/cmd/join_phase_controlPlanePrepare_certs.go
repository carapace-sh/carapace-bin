...
func init() {
	...
	join_phase_controlPlanePrepare_certsCmd.Flags().String("config", "", "Path to kubeadm config file.")
	...
	join_phase_controlPlanePrepare_certsCmd.Flags().StringSlice("discovery-token-ca-cert-hash", []string{}, "For token-based discovery, validate that the root CA public key matches this hash (format: \"<type>:<value>\").")
