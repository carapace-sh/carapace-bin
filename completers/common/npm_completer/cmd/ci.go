...
func init() {
	...
	ciCmd.Flags().Bool("no-audit", false, "skip audit")
	ciCmd.Flags().Bool("no-bin-links", false, "skip symlinks for package executables")
	ciCmd.Flags().Bool("no-fund", false, "skip funding message")
