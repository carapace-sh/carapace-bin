...
func init() {
	...
	evologCmd.Flags().BoolP("no-graph", "G", false, "Show a flat list of revisions instead of a graph")
	...
	evologCmd.Flags().StringSlice("revision", []string{"@"}, "Follow changes from these revisions")
	evologCmd.Flags().StringSliceP("revisions", "r", []string{"@"}, "Follow changes from these revisions")
	...
	evologCmd.Flag("revision").Hidden = true
