...
func init() {
	...
	logCmd.Flags().BoolP("no-graph", "G", false, "Show a flat list of revisions instead of a graph")
	...
	logCmd.Flags().StringSliceP("revision", "r", []string{"builtin_log()"}, "Which revisions to show")
	logCmd.Flags().StringSlice("revisions", []string{"builtin_log()"}, "Which revisions to show")
	...
	logCmd.Flag("revisions").Hidden = true
	...
	carapace.Gen(logCmd).PositionalAnyCompletion(
		carapace.ActionCallback(func(c carapace.Context) carapace.Action {
			revisions, err := logCmd.Flags().GetStringSlice("revision") // TODO use revsets.log if set and flag is not set
			if err != nil {
