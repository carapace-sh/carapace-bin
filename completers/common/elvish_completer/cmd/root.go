...
func init() {
	...
	rootCmd.Flags().StringS("deprecation-level", "deprecation-level", "", "Show warnings for all features deprecated as of version 0.X (default 21)")
	rootCmd.Flags().BoolS("help", "help", false, "Show usage help and quit")
	rootCmd.Flags().BoolS("i", "i", false, "A no-op flag, introduced for compatibility")
	rootCmd.Flags().BoolS("json", "json", false, "Show the output from -buildinfo, -compileonly or -version in JSON")
	rootCmd.Flags().BoolS("l", "l", false, "A no-op flag, introduced for compatibility")
