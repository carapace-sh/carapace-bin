...
func init() {
	...
	rootCmd.Flags().BoolP("binary", "b", false, "force binary mode on Windows")
	rootCmd.Flags().Bool("build-configuration", false, "output the build configuration of jq and exit")
	...
	rootCmd.Flags().String("library-path", "", "prepend a directory to the module search path")
	...
	rootCmd.Flags().Bool("raw-output0", false, "like -r but print NUL instead of newline after each output")
	...
	rootCmd.Flags().Bool("stream-errors", false, "like --stream, but invalid JSON inputs yield array values")
