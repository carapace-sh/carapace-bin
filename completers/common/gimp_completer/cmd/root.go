...
func init() {
	...
	rootCmd.Flags().Bool("quit", false, "Quit immediately after performing requested actions")
	rootCmd.Flags().String("screen", "", "X screen to use")
	rootCmd.Flags().String("session", "", "Use an alternate sessionrc file")
	rootCmd.Flags().Bool("show-debug-menu", false, "Show an image submenu with debug actions")
