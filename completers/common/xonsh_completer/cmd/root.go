...
func init() {
	...
	rootCmd.Flags().Bool("load-origin-env", false, "Load origin environment variables that were saved before running xonsh by using --save-origin-env")
	rootCmd.Flags().BoolP("login", "l", false, "Run as a login shell.")
	rootCmd.Flags().Bool("no-env", false, "Do not inherit parent environment variables.")
	...
	rootCmd.Flags().Bool("save-origin-env", false, "Save origin environment variables before running xonsh")
