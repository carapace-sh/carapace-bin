...
func init() {
	...
	rootCmd.Flags().StringS("D", "D", "", "set the start address of data symbols")
	...
	rootCmd.Flags().StringS("R", "R", "", "set address rounding quantum")
	rootCmd.Flags().StringS("T", "T", "", "set the start address of text symbols")
	...
	rootCmd.Flags().BoolS("a", "a", false, "no-op (deprecated)")
	rootCmd.Flags().BoolS("asan", "asan", false, "enable ASan interface")
	rootCmd.Flags().BoolS("aslr", "aslr", false, "enable ASLR for buildmode=c-shared on windows")
	...
	rootCmd.Flags().BoolS("bindnow", "bindnow", false, "mark a dynamically linked ELF object for immediate function binding")
	...
	rootCmd.Flags().BoolS("checklinkname", "checklinkname", false, "check linkname symbol references")
	rootCmd.Flags().BoolS("compressdwarf", "compressdwarf", false, "compress DWARF if possible")
	...
	rootCmd.Flags().StringS("fipso", "fipso", "", "write fips module to file")
	rootCmd.Flags().StringS("funcalign", "funcalign", "", "set function align to N bytes")
	...
	rootCmd.Flags().StringS("macos", "macos", "", "mac OS version to write in build info")
	rootCmd.Flags().StringS("macsdk", "macsdk", "", "mac SDK version to write in build info")
	...
	rootCmd.Flags().BoolS("n", "n", false, "no-op (deprecated)")
	rootCmd.Flags().StringS("o", "o", "", "write output to file")
	rootCmd.Flags().StringS("pluginpath", "pluginpath", "", "full path name for plugin")
	rootCmd.Flags().BoolS("pruneweakmap", "pruneweakmap", false, "prune weak mapinit refs")
	...
	rootCmd.Flags().StringS("randlayout", "randlayout", "", "randomize function layout")
