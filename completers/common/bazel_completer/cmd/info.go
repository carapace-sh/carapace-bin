...
func init() {
	...
	infoCmd.Flags().Bool("allow_analysis_cache_discard", false, "If discarding the analysis cache due to a change in the build system, setting this")
	infoCmd.Flags().Bool("allow_one_action_on_resource_unavailable", false, "If set, allow at least one action to run even if the resource is not enough or unavailable.")
	infoCmd.Flags().String("allowed_local_actions_regex", "", "A regex whitelist for action types which may be run locally. If unset, all actions are allowed to execute locally")
	infoCmd.Flags().StringSlice("allowed_strategies_by_exec_platform", nil, "Filters spawn strategies by the execution platform without affecting order.")
	infoCmd.Flags().Bool("analyze", false, "Execute the loading/analysis phase; this is the usual behaviour. Specifying `--noanalyze`")
	infoCmd.Flags().StringSlice("aspects", nil, "Comma-separated list of aspects to be applied to top-level targets. In the list, if")
	infoCmd.Flags().StringSlice("aspects_parameters", nil, "Specifies the values of the command-line aspects parameters. Each parameter value is")
	infoCmd.Flags().Bool("build", false, "Execute the build; this is the usual behaviour.")
	infoCmd.Flags().String("build_event_upload_max_retries", "", "The maximum number of times Bazel should retry uploading a build event.")
	...
	infoCmd.Flags().String("default_visibility", "", "Default visibility for packages that don't set it explicitly ('public' or 'private').")
	infoCmd.Flags().StringSlice("deleted_packages", nil, "A comma-separated list of names of packages which the build system will consider non-existent, even if they are visible somewhere on the package path.")
	infoCmd.Flags().String("directory_creation_cache", "", "Describes the cache used to store known regular directories as they're created. Parent directories of output files are created on-demand during action execution.")
	...
	infoCmd.Flags().String("experimental_active_directories", "", "Active directories for Skyfocus and remote analysis caching. Specify as comma-separated workspace root-relative paths. This is a stateful flag. Defining one persists it for subsequent invocations, until it is redefined with a new set.")
	infoCmd.Flags().Bool("experimental_analysis_cache_enable_metadata_queries", false, "A flag to switch on/off inserting and querying the metadata db (b/425247333).")
	infoCmd.Flags().String("experimental_analysis_cache_key_distinguisher_for_testing", "", "An opaque string used as part of the cache key. Should only be used for testing.")
	infoCmd.Flags().String("experimental_analysis_cache_server_checksum_override", "", "If set, Blaze will use this checksum to look up entries in the remote analysis cache and not its own. WARNING: this might result in incorrect behavior. Only for debugging. It's best if the difference between the writer and the reader is only additional logging. In particular, the data structures that are being serialized  and the observable behavior of the serialization machinery must not change.")
	infoCmd.Flags().String("experimental_analysis_cache_service", "", "Locator for the AnalysisCacheService instance.")
	infoCmd.Flags().String("experimental_aquery_dump_after_build_format", "", "Writes the state of Skyframe (which includes previous invocations on this blaze instance")
	infoCmd.Flags().String("experimental_aquery_dump_after_build_output_file", "", "Specify the output file for the aquery dump after a build. Use in conjunction with")
	infoCmd.Flags().Bool("experimental_async_execution", false, "If set to true, Bazel is allowed to run action in a virtual thread. The number of")
	infoCmd.Flags().String("experimental_async_execution_max_concurrent_actions", "", "The number of maximum concurrent actions to run with async execution. If the value is")
	...
	infoCmd.Flags().StringSlice("experimental_build_event_output_group_mode", nil, "Specify how an output group's files will be represented in `TargetComplete`/`AspectComplete`")
	infoCmd.Flags().String("experimental_build_event_upload_retry_minimum_delay", "", "Initial, minimum delay for exponential backoff retries when BEP upload fails. (exponent: 1.6)")
	infoCmd.Flags().String("experimental_build_event_upload_strategy", "", "Selects how to upload artifacts referenced in the build event protocol. In Bazel")
	infoCmd.Flags().Bool("experimental_check_external_other_files", false, "Check for modifications made to the non-output, non-repo external files, e.g. host files.")
	...
	infoCmd.Flags().String("experimental_convenience_symlinks", "", "This flag controls how the convenience symlinks (the symlinks that appear in the")
	infoCmd.Flags().Bool("experimental_convenience_symlinks_bep_event", false, "This flag controls whether or not we will post the build event")
	...
	infoCmd.Flags().String("experimental_cpu_load_scheduling_window_size", "", "The size of window during experimental scheduling of action based on CPU load. Make sense to define only when flag --experimental_cpu_load_scheduling is enabled.")
	...
	infoCmd.Flags().Bool("experimental_enable_skyfocus", false, "If true, enable the use of --experimental_active_directories to reduce Bazel's memory footprint for incremental builds. This feature is known as Skyfocus.")
	infoCmd.Flags().String("experimental_extra_action_filter", "", "Deprecated in favor of aspects. Filters set of targets to schedule extra_actions for.")
	...
	infoCmd.Flags().String("experimental_frontier_violation_check", "", "Strategies to handle potential incorrectness from changes beyond the frontier (i.e. outside the active directories)")
	infoCmd.Flags().Bool("experimental_frontier_violation_verbose", false, "If true, Bazel will print instructions for fixing Skycache violations")
	infoCmd.Flags().String("experimental_fsvc_threads", "", "The number of threads that are used by the `FileSystemValueChecker`.")
	infoCmd.Flags().String("experimental_lazy_macro_expansion_packages", "", "List of packages in which symbolic macro are expanded only if necessary.")
	...
	infoCmd.Flags().String("experimental_local_retries_on_crash", "", "Number of times to retry a local action when we detect that it crashed. This exists to workaround a bug in OSXFUSE which is tickled by the use of the dynamic scheduler and --experimental_local_lockfree_output due to constant process churn. The bug can be triggered by a cancelled process that ran *before* the process we are trying to run, introducing corruption in its file reads.")
	...
	infoCmd.Flags().String("experimental_max_directories_to_eagerly_visit_in_globbing", "", "If non-negative, the first time a glob is evaluated in a package, the subdirectories of the package will be traversed in order to warm filesystem caches and compensate for lack of parallelism in globbing. At most this many directories will be visited.")
	...
	infoCmd.Flags().String("experimental_oom_sensitive_skyfunctions_semaphore_size", "", "Sets the size of the semaphore used to prevent SkyFunctions with large peak memory requirement from OOM-ing blaze. A value of 0 indicates that no semaphore should be used. Example value: `HOST_CPUS*0.5`.")
	...
	infoCmd.Flags().String("experimental_remote_analysis_cache", "", "The URL for the remote analysis caching backend.")
	infoCmd.Flags().String("experimental_remote_analysis_cache_concurrency", "", "Target concurrency for remote analysis caching RPCs.")
	infoCmd.Flags().String("experimental_remote_analysis_cache_deadline", "", "Deadline to use for remote analysis cache operations.")
	infoCmd.Flags().String("experimental_remote_analysis_cache_max_batch_size", "", "Batch size limit for remote analysis caching RPCs.")
	infoCmd.Flags().String("experimental_remote_analysis_cache_mode", "", "The transport direction for the remote analysis cache.")
	infoCmd.Flags().String("experimental_remote_analysis_unreachable_cache_retry_interval", "", "How long to wait before retrying a cache get request that failed due to an UNREACHABLE channel. This is a workaround for the client library reporting 'ready' prematurely.")
	infoCmd.Flags().String("experimental_remote_analysis_write_proxy", "", "The address of the SkycacheStorageWriteProxyService. If set, this service will be used for uploading analysis cache data.")
	infoCmd.Flags().String("experimental_remote_cache_eviction_retries", "", "The maximum number of attempts to retry if the build encountered a transient remote cache error that would otherwise fail the build. Applies for example when artifacts are evicted from the remote cache, or in certain cache failure conditions. A new invocation id will be generated for each attempt.")
	...
	infoCmd.Flags().String("experimental_skyfocus_dump_keys", "", "For debugging Skyfocus. Dump the focused SkyKeys (roots, leafs, focused deps, focused rdeps).")
	...
	infoCmd.Flags().String("experimental_skyframe_cpu_heavy_skykeys_thread_pool_size", "", "If set to a positive value (e.g. `HOST_CPUS*1.5`), Skyframe will run the loading/analysis phase with 2 separate thread pools: 1 with `<value>` threads (ideally close to `HOST_CPUS`) reserved for CPU-heavy SkyKeys, and 1 \"standard\" thread pool (whose size is controlled by `--loading_phase_threads`) for the rest.")
	infoCmd.Flags().Bool("experimental_skyframe_error_handling_refactor", false, "Used solely for the safe rollout of simplifying Skyframe error handling. This will beremoved once the rollout is complete (expected timeframe: 1 release)")
	infoCmd.Flags().String("experimental_skyframe_memory_dump", "", "Dump the memory use of individual nodes in the Skyframe graph after the build. This")
	...
	infoCmd.Flags().String("experimental_skymeld_analysis_overlap_percentage", "", "The value represents the % of the analysis phase which will be overlapped with the execution phase. A value of x means Skyframe will queue up execution tasks and wait until there's x% of the top level target left to be analyzed before allowing them to launch. When the value is 0%, we'd wait for all analysis to finish before executing (no overlap). When it's 100%, the phases are free to overlap as much as they can.")
	...
	infoCmd.Flags().String("explain", "", "Causes the build system to explain each executed step of the build. The explanation is written to the specified log file.")
	...
	infoCmd.Flags().Bool("incompatible_skip_genfiles_symlink", false, "If set to true, the genfiles symlink will not be created. For more information, see")
	infoCmd.Flags().String("info_output_type", "", "If stdout, results are directly printed to the console. If response_proto, the info command results are packed in response extensions.")
	infoCmd.Flags().StringP("jobs", "j", "", "The number of concurrent jobs to run. Takes an integer, or a keyword (\"auto\", \"HOST_CPUS\", \"HOST_RAM\"), optionally followed by an operation ([-|*]<float>) eg. \"auto\", \"HOST_CPUS*.5\". Values must be between 1 and 5000. Values above 2500 may cause memory issues. \"auto\" calculates a reasonable default based on host resources.")
	...
	infoCmd.Flags().String("legacy_globbing_threads", "", "Number of threads to use for glob evaluation. Takes an integer, or a keyword (\"auto\", \"HOST_CPUS\", \"HOST_RAM\"), optionally followed by an operation ([-|*]<float>) eg. \"auto\", \"HOST_CPUS*.5\". \"auto\" means to use a reasonable value derived from the machine's hardware profile (e.g. the number of processors).")
	infoCmd.Flags().Bool("legacy_important_outputs", false, "Use this to suppress generation of the legacy `important_outputs` field in the")
	infoCmd.Flags().String("loading_phase_threads", "", "Number of parallel threads to use for the loading/analysis phase.Takes an integer, or a keyword (\"auto\", \"HOST_CPUS\", \"HOST_RAM\"), optionally followed by an operation ([-|*]<float>) eg. \"auto\", \"HOST_CPUS*.5\". \"auto\" sets a reasonable default based on host resources. Must be at least 1.")
	infoCmd.Flags().StringSlice("local_resources", nil, "Set the number of resources available to Bazel. Takes in an assignment to a float or HOST_RAM/HOST_CPUS, optionally followed by [-|*]<float> (eg. memory=HOST_RAM*.5 to use half the available RAM). Can be used multiple times to specify multiple types of resources. Bazel will limit concurrently running actions based on the available resources and the resources required. Tests can declare the amount of resources they need by using a tag of the \"resources:<resource name>:<amount>\" format. ")
	infoCmd.Flags().String("local_termination_grace_seconds", "", "Time to wait between terminating a local process due to timeout and forcefully shutting it down.")
	infoCmd.Flags().String("local_test_jobs", "", "The max number of local test jobs to run concurrently. Takes an integer, or a keyword (\"auto\", \"HOST_CPUS\", \"HOST_RAM\"), optionally followed by an operation ([-|*]<float>) eg. \"auto\", \"HOST_CPUS*.5\". 0 means local resources will limit the number of local test jobs to run concurrently instead. Setting this greater than the value for --jobs is ineffectual.")
	infoCmd.Flags().Bool("materialize_param_files", false, "Writes intermediate parameter files to output tree even when using remote action execution or caching. Useful when debugging actions. This is implied by --subcommands and --verbose_failures.")
	infoCmd.Flags().String("max_config_changes_to_show", "", "When discarding the analysis cache due to a change in the build options, displays up to the given number of changed option names. If the number given is -1, all changed options will be displayed.")
	infoCmd.Flags().String("max_test_output_bytes", "", "Specifies maximum per-test-log size that can be emitted when --test_output is 'errors' or 'all'. Useful for avoiding overwhelming the output with excessively noisy test output. The test header is included in the log size. Negative values imply no limit. Output is all or nothing.")
	infoCmd.Flags().Bool("noallow_analysis_cache_discard", false, "If discarding the analysis cache due to a change in the build system, setting this")
	infoCmd.Flags().Bool("noallow_one_action_on_resource_unavailable", false, "If set, allow at least one action to run even if the resource is not enough or unavailable.")
	infoCmd.Flags().Bool("noanalyze", false, "Execute the loading/analysis phase; this is the usual behaviour. Specifying `--noanalyze`")
	infoCmd.Flags().Bool("nobuild", false, "Execute the build; this is the usual behaviour.")
	...
	infoCmd.Flags().Bool("noexperimental_analysis_cache_enable_metadata_queries", false, "A flag to switch on/off inserting and querying the metadata db (b/425247333).")
	infoCmd.Flags().Bool("noexperimental_async_execution", false, "If set to true, Bazel is allowed to run action in a virtual thread. The number of")
	...
	infoCmd.Flags().Bool("noexperimental_check_external_other_files", false, "Check for modifications made to the non-output, non-repo external files, e.g. host files.")
	infoCmd.Flags().Bool("noexperimental_check_output_files", false, "Check for modifications made to the output files of a build. Consider setting this flag to false if you don't expect these files to change outside of bazel since it will speed up subsequent runs as they won't have to check a previous run's cache.")
	infoCmd.Flags().Bool("noexperimental_convenience_symlinks_bep_event", false, "This flag controls whether or not we will post the build event")
	...
	infoCmd.Flags().Bool("noexperimental_enable_skyfocus", false, "If true, enable the use of --experimental_active_directories to reduce Bazel's memory footprint for incremental builds. This feature is known as Skyfocus.")
	...
	infoCmd.Flags().Bool("noexperimental_frontier_violation_verbose", false, "If true, Bazel will print instructions for fixing Skycache violations")
	...
	infoCmd.Flags().Bool("noexperimental_skyframe_error_handling_refactor", false, "Used solely for the safe rollout of simplifying Skyframe error handling. This will beremoved once the rollout is complete (expected timeframe: 1 release)")
	...
	infoCmd.Flags().Bool("noincompatible_skip_genfiles_symlink", false, "If set to true, the genfiles symlink will not be created. For more information, see")
	...
	infoCmd.Flags().Bool("nolegacy_important_outputs", false, "Use this to suppress generation of the legacy `important_outputs` field in the")
	infoCmd.Flags().Bool("nomaterialize_param_files", false, "Writes intermediate parameter files to output tree even when using remote action execution or caching. Useful when debugging actions. This is implied by --subcommands and --verbose_failures.")
	...
	infoCmd.Flags().Bool("norun_validations", false, "Whether to run validation actions as part of the build. See [Validation Actions].")
	...
	infoCmd.Flags().Bool("noskip_incompatible_explicit_targets", false, "Skip incompatible targets that are explicitly listed on the command line.")
	...
	infoCmd.Flags().String("output_filter", "", "Only shows warnings and action outputs for rules with a name matching the provided regular expression.")
	infoCmd.Flags().StringSlice("output_groups", nil, "A list of comma-separated output group names, each of which optionally prefixed by a `+`")
	...
	infoCmd.Flags().String("package_path", "", "A colon-separated list of where to look for packages. Elements beginning with '%workspace%' are relative to the enclosing workspace. If omitted or empty, the default is the output of 'bazel info default-package-path'.")
	infoCmd.Flags().String("progress_report_interval", "", "The number of seconds to wait between reports on still running jobs. The")
	infoCmd.Flags().String("remote_analysis_json_log", "", "If set, a JSON file is written to this location that contains a detailed log of the behavior of remote analysis caching. It's interpreted as a path relative to the current working directory.")
	infoCmd.Flags().Bool("rewind_lost_inputs", false, "Whether to use action rewinding to recover from lost inputs.")
	infoCmd.Flags().Bool("run_validations", false, "Whether to run validation actions as part of the build. See [Validation Actions].")
	infoCmd.Flags().String("serialized_frontier_profile", "", "Dump a profile of serialized frontier bytes. Specifies the output path.")
	...
	infoCmd.Flags().String("test_lang_filters", "", "Specifies a comma-separated list of test languages. Each language can be optionally preceded with '-' to specify excluded languages. Only those test targets will be found that are written in the specified languages. The name used for each language should be the same as the language prefix in the *_test rule, e.g. one of 'cc', 'java', 'py', etc. This option affects --build_tests_only behavior and the test command.")
	infoCmd.Flags().String("test_output", "", "Specifies desired output mode. Not to be confused with `--test_summary` which controls")
	infoCmd.Flags().String("test_size_filters", "", "Specifies a comma-separated list of test sizes. Each size can be optionally preceded with '-' to specify excluded sizes. Only those test targets will be found that contain at least one included size and do not contain any excluded sizes. This option affects --build_tests_only behavior and the test command.")
	infoCmd.Flags().String("test_strategy", "", "Specifies which strategy to use when running tests.")
	infoCmd.Flags().String("test_summary", "", "Specifies the desired format of the test summary. Valid values are;")
	infoCmd.Flags().String("test_tag_filters", "", "Specifies a comma-separated list of test tags. Each tag can be optionally preceded with '-' to specify excluded tags. Only those test targets will be found that contain at least one included tag and do not contain any excluded tags. This option affects --build_tests_only behavior and the test command.")
	infoCmd.Flags().String("test_timeout_filters", "", "Specifies a comma-separated list of test timeouts. Each timeout can be optionally preceded with '-' to specify excluded timeouts. Only those test targets will be found that contain at least one included timeout and do not contain any excluded timeouts. This option affects --build_tests_only behavior and the test command.")
	infoCmd.Flags().String("test_tmpdir", "", "Specifies the base temporary directory for 'bazel test' to use.")
	...
	infoCmd.Flags().String("version_window_for_dirty_node_gc", "", "Nodes that have been dirty for more than this many versions will be deleted from the graph upon the next update. Values must be non-negative long integers, or -1 indicating the maximum possible window.")
