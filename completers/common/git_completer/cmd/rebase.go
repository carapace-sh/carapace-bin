...
func init() {
	...
	rebaseCmd.Flags().Bool("ff", false, "cherry-pick all commits, even if unchanged")
	...
	rebaseCmd.Flags().Bool("reset-author-date", false, "ignore author date and use current date")
	...
	rebaseCmd.Flags().Bool("stat", false, "show diffstat of what changed upstream")
	...
	rebaseCmd.Flags().StringArray("trailer", nil, "add custom trailer(s)")
	rebaseCmd.Flags().Bool("update-refs", false, "update branches that point to commits that are being rebased")
	rebaseCmd.Flags().BoolP("verbose", "v", false, "display a diffstat of what changed upstream")
	rebaseCmd.Flags().Bool("verify", false, "allow pre-rebase hook to run")
