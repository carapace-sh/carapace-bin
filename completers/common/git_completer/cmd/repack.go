...
func init() {
	...
	repackCmd.Flags().String("combine-cruft-below-size", "", "combine cruft packs smaller than this size")
	...
	repackCmd.Flags().String("filter-to", "", "pack prefix to store filtered-out objects")
	...
	repackCmd.Flags().String("max-cruft-size", "", "maximum size of each cruft pack")
	...
	repackCmd.Flags().String("name-hash-version", "", "name hash version for pack index")
	repackCmd.Flags().Bool("pack-kept-objects", false, "repack objects in packs marked with .keep")
	repackCmd.Flags().Bool("path-walk", false, "pass --path-walk to git-pack-objects")
	...
	carapace.Gen(repackCmd).FlagCompletion(carapace.ActionMap{
		...
		"filter-to":          carapace.ActionDirectories(),
