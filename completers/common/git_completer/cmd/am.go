...
func init() {
	...
	amCmd.Flags().Bool("allow-empty", false, "create an empty commit with the contents of the e-mail message as its log message")
	amCmd.Flags().Bool("committer-date-is-author-date", false, "use the author date as committer date")
	...
	amCmd.Flags().Bool("ignore-date", false, "use the committer date as author date")
	...
	amCmd.Flags().StringArray("include", nil, "pass it through git-apply")
	...
	amCmd.Flags().Bool("retry", false, "try to apply the last conflicting patch again")
	amCmd.Flags().BoolP("scissors", "c", false, "strip everything before a scissors line")
	amCmd.Flags().String("show-current-patch", "", "show the message at which git-am has stopped due to conflicts")
