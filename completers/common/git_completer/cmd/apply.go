...
func init() {
	...
	applyCmd.Flags().StringArray("exclude", nil, "don't apply changes matching the given path")
	...
	applyCmd.Flags().StringArray("include", nil, "apply changes matching the given path")
	...
	applyCmd.Flags().Bool("ours", false, "use our version for conflicting hunks")
	applyCmd.Flags().StringS("p", "p", "", "remove <num> leading slashes from traditional diff paths")
	applyCmd.Flags().BoolP("quiet", "q", false, "suppress report messages")
	...
	applyCmd.Flags().Bool("theirs", false, "use their version for conflicting hunks")
	applyCmd.Flags().Bool("unidiff-zero", false, "don't expect at least one line of context")
	applyCmd.Flags().Bool("union", false, "use union version for conflicting hunks")
