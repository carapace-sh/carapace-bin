...
func init() {
	...
	configCmd.Flags().Bool("append", false, "add a new line without altering any existing values")
	...
	configCmd.Flags().String("comment", "", "append a comment at the end of new or modified lines")
	...
	configCmd.Flags().Bool("no-show-names", false, "do not show config keys")
	configCmd.Flags().Bool("no-type", false, "un-sets the previously set type specifier")
	...
	configCmd.Flags().Bool("show-names", false, "show config keys in addition to their values")
	...
	configCmd.Flags().String("url", "", "show config matching the given URL")
	configCmd.Flags().String("value", "", "show config with values matching the pattern")
