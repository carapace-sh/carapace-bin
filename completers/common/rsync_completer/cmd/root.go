...
func init() {
	...
	rootCmd.Flags().String("compress-threads", "", "explicitly set compression threads (aka --zt)")
	...
	rootCmd.Flags().Bool("copy-devices", false, "copy device contents as a regular file")
	...
	rootCmd.Flags().Bool("fsync", false, "fsync every written file")
	...
	rootCmd.Flags().Bool("old-args", false, "disable the modern arg-protection idiom")
	rootCmd.Flags().Bool("old-dirs", false, "works like --dirs when talking to old rsync")
	...
	rootCmd.Flags().BoolP("secluded-args", "s", false, "use the protocol to safely send the args")
	...
	rootCmd.Flags().Bool("trust-sender", false, "trust the remote sender's file list")
