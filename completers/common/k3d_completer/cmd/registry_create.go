...
func init() {
	...
	registry_createCmd.Flags().Bool("delete-enabled", false, "Enable image deletion")
	registry_createCmd.Flags().Bool("enforce-port-match", false, "Make the internal registry port match the external one")
