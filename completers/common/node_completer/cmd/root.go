...
func init() {
	...
	rootCmd.Flags().Bool("allow-ffi", false, "allow use of FFI when any permissions are set")
	...
	rootCmd.Flags().Bool("allow-inspector", false, "allow use of inspector when any permissions are set")
	rootCmd.Flags().Bool("allow-net", false, "allow network access when any permissions are set")
	...
	rootCmd.Flags().Bool("build-sea", false, "Generate a single executable application from the current script")
	rootCmd.Flags().Bool("build-snapshot", false, "Generate a snapshot blob when the process exits")
	rootCmd.Flags().String("build-snapshot-config", "", "specify the path to the snapshot configuration file")
	...
	rootCmd.Flags().Bool("disable-wasm-trap-handler", false, "disable Wasm trap handler support")
	...
	rootCmd.Flags().Bool("experimental-ffi", false, "experimental FFI support")
	...
	rootCmd.Flags().Bool("experimental-storage-inspection", false, "experimental storage inspection support")
	rootCmd.Flags().Bool("experimental-stream-iter", false, "experimental async iterator support for streams")
	rootCmd.Flags().Bool("experimental-strip-types", false, "experimental type stripping for TypeScript files")
	...
	rootCmd.Flags().String("experimental-test-tag-filter", "", "run tests whose tags match this filter")
	...
	rootCmd.Flags().String("inspect-brk", "", "activate inspector on host:port and break before user code starts")
	...
	rootCmd.Flags().String("max-old-space-size", "", "set V8's max old space size in MB")
	...
	rootCmd.Flags().Bool("no-experimental-websocket", false, "experimental WebSocket API")
	...
	rootCmd.Flags().Bool("no-require-module", false, "disable loading synchronous ES Modules in require()")
	...
	rootCmd.Flags().Bool("permission-audit", false, "enable permission audit logging")
	...
	rootCmd.Flags().Bool("require-module", false, "Allow loading synchronous ES Modules in require()")
	...
	rootCmd.Flags().String("test-random-seed", "", "seed for randomizing test execution order")
	rootCmd.Flags().Bool("test-randomize", false, "randomize test execution order")
	...
	rootCmd.Flags().Bool("webstorage", false, "experimental Web Storage API")
	...
	carapace.Gen(rootCmd).FlagCompletion(carapace.ActionMap{
		...
		"heapsnapshot-signal":      ps.ActionKillSignals(false),
		...
		"report-signal":            ps.ActionKillSignals(false),
		...
		"watch-kill-signal":        ps.ActionKillSignals(false),
