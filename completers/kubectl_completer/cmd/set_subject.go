...
func init() {
	...
	set_subjectCmd.Flags().String("field-manager", "kubectl-set", "Name of the manager used to track field ownership.")
	set_subjectCmd.Flags().StringSliceP("filename", "f", []string{}, "Filename, directory, or URL to files the resource to update the subjects")
	set_subjectCmd.Flags().StringArray("group", []string{}, "Groups to bind to the role")
