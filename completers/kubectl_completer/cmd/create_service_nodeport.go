...
func init() {
	...
	create_service_nodeportCmd.Flags().Bool("allow-missing-template-keys", false, "If true, ignore any errors in templates when a field or map key is missing in the template. Only applies to golang and jsonpath output formats.")
	...
	create_service_nodeportCmd.Flags().String("validate", "", "Validation mode.")
