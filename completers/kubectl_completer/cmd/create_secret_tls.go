...
func init() {
	...
	create_secret_tlsCmd.Flags().String("dry-run", "none", "Must be \"none\", \"server\", or \"client\". If client strategy, only print the object that would be sent, without sending it. If server strategy, submit server-side request without persisting the resource.")
	create_secret_tlsCmd.Flags().String("field-manager", "kubectl-create", "Name of the manager used to track field ownership.")
	...
	create_secret_tlsCmd.Flags().String("validate", "strict", "Must be one of: strict (or true), warn, ignore (or false).")
	create_secret_tlsCmd.Flag("dry-run").NoOptDefVal = "unchanged"
	create_secret_tlsCmd.Flag("validate").NoOptDefVal = "strict"
