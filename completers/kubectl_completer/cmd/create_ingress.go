...
func init() {
	...
	create_ingressCmd.Flags().String("dry-run", "none", "Must be \"none\", \"server\", or \"client\". If client strategy, only print the object that would be sent, without sending it. If server strategy, submit server-side request without persisting the resource.")
	create_ingressCmd.Flags().String("field-manager", "kubectl-create", "Name of the manager used to track field ownership.")
	...
	create_ingressCmd.Flags().StringArray("rule", []string{}, "Rule in format host/path=service:port[,tls=secretname]. Paths containing the leading character '*' are considered pathType=Prefix. tls argument is optional.")
	...
	create_ingressCmd.Flags().String("validate", "strict", "Must be one of: strict (or true), warn, ignore (or false).")
	create_ingressCmd.Flag("dry-run").NoOptDefVal = "unchanged"
	create_ingressCmd.Flag("validate").NoOptDefVal = "strict"
