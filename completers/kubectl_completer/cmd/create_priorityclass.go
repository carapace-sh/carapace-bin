...
func init() {
	...
	create_priorityclassCmd.Flags().String("field-manager", "kubectl-create", "Name of the manager used to track field ownership.")
	...
	create_priorityclassCmd.Flags().String("preemption-policy", "PreemptLowerPriority", "preemption-policy is the policy for preempting pods with lower priority.")
	...
	create_priorityclassCmd.Flags().String("validate", "strict", "Must be one of: strict (or true), warn, ignore (or false).")
	create_priorityclassCmd.Flags().Int32("value", 0, "the value of this priority class.")
	create_priorityclassCmd.Flag("dry-run").NoOptDefVal = "unchanged"
	create_priorityclassCmd.Flag("validate").NoOptDefVal = "strict"
