...
func init() {
	...
	create_roleCmd.Flags().StringArray("resource-name", []string{}, "Resource in the white list that the rule applies to, repeat this flag for multiple items")
	...
	create_roleCmd.Flags().String("validate", "strict", "Must be one of: strict (or true), warn, ignore (or false).")
	create_roleCmd.Flags().StringSlice("verb", []string{}, "Verb that applies to the resources contained in the rule")
	create_roleCmd.Flag("dry-run").NoOptDefVal = "unchanged"
	create_roleCmd.Flag("validate").NoOptDefVal = "strict"
