...
func init() {
	...
	rootCmd.Flags().String("bazelrc", "", "<path>The location of the user .bazelrc file containing default values of Bazel options. /dev/null indicates that all further `--bazelrc`s will be ignored, which is useful to disable the search for a user rc file, e.g. in release builds.")
	...
	rootCmd.Flags().Bool("command_port", false, "Port to start up the gRPC command server on. If 0, let the kernel choose.")
	rootCmd.Flags().Bool("connect_timeout_secs", false, "The amount of time the client waits for each attempt to connect to the server")
	rootCmd.Flags().Bool("default_system_javabase", false, "The root of the user's local JDK install, to be used as the default target javabase and as a fall-back host_javabase. This is not the embedded JDK.")
	rootCmd.Flags().Bool("digest_function", false, "The hash function to use when computing file digests.")
	...
	rootCmd.Flags().String("host_jvm_args", "", "<jvm_arg>Flags to pass to the JVM executing Blaze.")
	rootCmd.Flags().Bool("host_jvm_debug", false, "Convenience option to add some additional JVM startup flags, which cause the JVM to wait during startup until you connect from a JDWP-compliant debugger (like Eclipse) to port 5005.")
	...
	rootCmd.Flags().Bool("install_base", false, "This launcher option is intended for use only by tests.")
	rootCmd.Flags().Bool("install_md5", false, "This launcher option is intended for use only by tests.")
	rootCmd.Flags().Bool("invocation_policy", false, "A base64-encoded-binary-serialized or text-formated invocation_policy.InvocationPolicy proto. Unlike other options, it is an error to specify --invocation_policy multiple times.")
	...
	rootCmd.Flags().Bool("local_startup_timeout_secs", false, "The maximum amount of time the client waits to connect to the server")
	...
	rootCmd.Flags().Bool("noshutdown_on_low_sys_mem", false, "If max_idle_secs is set and the build server has been idle for a while, shut down the server when the system is low on free RAM. Linux only.")
	...
	rootCmd.Flags().Bool("nowatchfs", false, "If true, %{product} tries to use the operating system's file watch service for local changes instead of scanning every file for a change.")
	...
	rootCmd.Flags().Bool("nowrite_command_log", false, "Whether or not to write the command.log file")
	rootCmd.Flags().Bool("option_sources", false, "")
	...
	rootCmd.Flags().Bool("product_name", false, "The name of the build system. It is used as part of the name of the generated directories (e.g. productName-bin for binaries) as well as for printing error messages and logging")
	...
	rootCmd.Flags().Bool("shutdown_on_low_sys_mem", false, "If max_idle_secs is set and the build server has been idle for a while, shut down the server when the system is low on free RAM. Linux only.")
	rootCmd.Flags().Bool("system_rc", false, "Whether or not to look for the system-wide bazelrc.")
	rootCmd.Flags().Bool("unix_digest_hash_attribute_name", false, "The name of an extended attribute that can be placed on files to store a precomputed copy of the file's hash, corresponding with --digest_function. This option can be used to reduce disk I/O and CPU load caused by hash computation. This extended attribute is checked on all source files and output files, meaning that it causes a significant number of invocations of the getxattr() system call.")
	rootCmd.Flags().Bool("unlimit_coredumps", false, "Raises the soft coredump limit to the hard limit to make coredumps of the server (including the JVM) and the client possible under common conditions. Stick this flag in your bazelrc once and forget about it so that you get coredumps when you actually encounter a condition that triggers them.")
	rootCmd.Flags().Bool("watchfs", false, "If true, %{product} tries to use the operating system's file watch service for local changes instead of scanning every file for a change.")
	...
	rootCmd.Flags().Bool("workspace_directory", false, "The root of the workspace, that is, the directory that Bazel uses as the root of the build. This flag is only to be set by the bazel client.")
	rootCmd.Flags().Bool("workspace_rc", false, "Whether or not to look for the workspace bazelrc file at $workspace/.bazelrc")
	rootCmd.Flags().Bool("write_command_log", false, "Whether or not to write the command.log file")
}
