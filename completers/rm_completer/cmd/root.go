...
func init() {
	...
	rootCmd.Flags().BoolP("I", "I", false, "prompt once before removing more than three files, or when removing recursively")
	...
	rootCmd.Flags().BoolP("i", "i", false, "prompt before every removal")
	rootCmd.Flags().Bool("interactive", false, "prompt according to WHEN: never, once (-I), or always (-i); without WHEN, prompt always")
