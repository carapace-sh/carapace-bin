...
func init() {
	...
	formattersCmd.Flags().StringSliceP("disable", "D", []string{}, "Disable specific linter")
	formattersCmd.Flags().StringSliceP("enable", "E", []string{}, "Enable specific linter")
	formattersCmd.Flags().StringSlice("enable-only", []string{}, "Override linters configuration section to only run the specific linter(s)")
