...
func init() {
	...
	testCmd.Flags().Bool("[no-]null-assertions", false, "Perform additional null assertions on the boundaries of migrated and un-migrated code.")
	testCmd.Flags().Bool("[no-]pub", false, "Whether to run \"flutter pub get\" before executing this command.")
	testCmd.Flags().Bool("[no-]track-widget-creation", false, "Track widget creation locations.")
	...
	testCmd.Flags().String("dart-define", "", "Additional key-value pairs that will be available as constants.")
