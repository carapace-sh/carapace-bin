...
func init() {
	...
	rootCmd.Flags().Bool("beneath", false, "ignore mount propagation from parent mount")
	...
	rootCmd.Flags().String("map-groups", "", "remap GID of source from inner to outer")
	rootCmd.Flags().String("map-users", "", "remap UID of source from inner to outer")
	rootCmd.Flags().StringP("mkdir", "m", "", "create a mountpoint of the given mode if it doesn't exist")
	...
	rootCmd.Flags().Bool("onlyonce", false, "ensure the same source is not mounted more than once")
	...
	rootCmd.Flags().Bool("read-write", false, "mount the filesystem read-write (default)")
	rootCmd.Flags().Bool("ro", false, "mount the filesystem read-only (same as -o ro)")
