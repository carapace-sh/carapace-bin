...
func init() {
	...
	rootCmd.Flags().BoolP("archive", "a", false, "Archive mode. Same as -RLpP")
	rootCmd.Flags().BoolP("clipboard", "c", false, "Copy the file data to the pasteboard")
	rootCmd.Flags().BoolP("follow-all-symlinks", "L", false, "Follow all symbolic links encountered during traversal (with -R)")
	rootCmd.Flags().BoolP("follow-symlinks", "H", false, "Follow symbolic links encountered as arguments (with -R)")
	...
	rootCmd.Flags().BoolP("no-dereference", "P", false, "Do not follow symbolic links (with -R)")
	rootCmd.Flags().BoolP("no-preserve-extended-attributes", "N", false, "Do not preserve extended attributes and ACLs")
	rootCmd.Flags().BoolP("no-traverse", "X", false, "Do not copy directories that are on a different filesystem (with -R)")
	...
	rootCmd.Flags().BoolP("sparse", "S", false, "Attempt to create sparse files in the destination")
	rootCmd.Flags().BoolP("verbose", "v", false, "Cause cp to be verbose, showing files as they are copied")
	rootCmd.Flags().BoolS("x", "x", false, "Copy file content only, not extended attributes")
