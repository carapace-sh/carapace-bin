...
func init() {
	...
	rootCmd.Flags().BoolS("U", "U", false, "Show interactive toolbar in interactive mode")
	rootCmd.Flags().BoolS("W", "W", false, "Start interaction in window selection mode")
	rootCmd.Flags().BoolP("audio-capture", "g", false, "Capture audio using default input during video recording")
	rootCmd.Flags().StringP("audio-source", "G", "", "Capture audio using specified audio source ID during video recording")
	rootCmd.Flags().StringP("bundleid", "B", "", "Open screen capture output in app with bundle id")
	rootCmd.Flags().BoolP("clicks", "k", false, "Show clicks in video recording mode")
	...
	rootCmd.Flags().BoolP("no-dpi", "r", false, "Do not add screen dpi meta data to captured file")
	...
	rootCmd.Flags().BoolP("ui", "u", false, "Present UI after screencapture is complete")
	rootCmd.Flags().StringP("video", "V", "", "Capture a video recording for <seconds>")
	rootCmd.Flags().BoolP("video-recording", "v", false, "Capture video recording of the screen")
