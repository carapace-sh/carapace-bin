...
func ActionRevsetFunctions(brackets bool) carapace.Action {
	return carapace.ActionCallback(func(c carapace.Context) carapace.Action {
		...
		withArgs := carapace.ActionValuesDescribed(
			...
			"diff_lines_added", "like `diff_lines()` above, but matches only the \"added\" side of the diff",
			"diff_lines", "Commits containing diffs matching the given `text` pattern line by line",
			"diff_lines_removed", "like `diff_lines()` above, but matches only the \"removed\" side of the diff",
